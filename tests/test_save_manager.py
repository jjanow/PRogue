...
def make_save_game(mock_stdscr):
...
class TestSerializeMap:
    def test_round_trip(self, tmp_path, mock_stdscr):
        sm = SaveManager(save_dir=str(tmp_path))
        original = [['#', '.', '#'], ['.', '.', '.']]
        serialized = sm._serialize_map(original)
        restored = sm._deserialize_map(serialized)
        assert restored == original

    def test_empty_map_round_trip(self, tmp_path, mock_stdscr):
        sm = SaveManager(save_dir=str(tmp_path))
        assert sm._serialize_map([]) == ""
        assert sm._deserialize_map("") == []
...
class TestSerialize2DArray:
    def test_boolean_round_trip(self, tmp_path):
        ...
        serialized = sm._serialize_2d_array(original)
        restored = sm._deserialize_2d_array(serialized, width=3, height=2)
        assert restored == original

    def test_empty_array(self, tmp_path):
        sm = SaveManager(save_dir=str(tmp_path))
        assert sm._serialize_2d_array([]) == ""


class TestSerializeInventory:
    def test_round_trip_with_consumable(self, tmp_path):
        sm = SaveManager(save_dir=str(tmp_path))
        potion = Item('Red Potion', lambda e: e.heal(20),
                      value=20, effect_type='heal', gold_value=50)
        inv = Counter({potion: 3})
        serialized = sm._serialize_inventory(inv)
        restored = sm._deserialize_inventory(serialized)
        names = {item.name: count for item, count in restored.items()}
        assert names.get('Red Potion') == 3

    def test_round_trip_with_equipment(self, tmp_path):
        sm = SaveManager(save_dir=str(tmp_path))
...
class TestSerializeEquipment:
    def test_empty_slots_round_trip(self, tmp_path):
        sm = SaveManager(save_dir=str(tmp_path))
        entity = Entity(0, 0, '@', 'P', 100, 0, 0)
        serialized = sm._serialize_equipment(entity.equipment)
        restored = sm._deserialize_equipment(serialized)
        # All slots should still exist with item=None
        assert 'a' in restored
        assert restored['a']['item'] is None

    def test_equipped_item_round_trip(self, tmp_path):
        sm = SaveManager(save_dir=str(tmp_path))
        entity = Entity(0, 0, '@', 'P', 100, 0, 0)
...
class TestSaveGame:
    def test_creates_save_file(self, tmp_path, mock_stdscr):
        game = make_save_game(mock_stdscr)
        sm = SaveManager(save_dir=str(tmp_path))
        sm.save_game(game, slot=1)
        assert (tmp_path / 'save_1.json').exists()

    def test_save_file_is_valid_json(self, tmp_path, mock_stdscr):
        game = make_save_game(mock_stdscr)
        sm = SaveManager(save_dir=str(tmp_path))
        sm.save_game(game, slot=1)
    ...
    def test_invalid_slot_raises(self, tmp_path, mock_stdscr):
        game = make_save_game(mock_stdscr)
        sm = SaveManager(save_dir=str(tmp_path))
...
class TestLoadGame:
    def _save_and_load(self, tmp_path, mock_stdscr, mutate=None):
    ...
    def test_player_name_preserved(self, tmp_path, mock_stdscr):
        def mutate(g): g.player.name = 'Percival'
        _, loaded = self._save_and_load(tmp_path, mock_stdscr, mutate)
        assert loaded.player.name == 'Percival'

    def test_player_stats_preserved(self, tmp_path, mock_stdscr):
        def mutate(g):
    ...
    def test_player_health_preserved(self, tmp_path, mock_stdscr):
        def mutate(g):
    ...
    def test_player_position_preserved(self, tmp_path, mock_stdscr):
        _, loaded = self._save_and_load(tmp_path, mock_stdscr)
        assert loaded.player.x == 5
        assert loaded.player.y == 4

    def test_player_money_preserved(self, tmp_path, mock_stdscr):
        def mutate(g): g.player.money = 1234
        _, loaded = self._save_and_load(tmp_path, mock_stdscr, mutate)
        assert loaded.player.money == 1234

    def test_dungeon_level_preserved(self, tmp_path, mock_stdscr):
        def mutate(g): g.dungeon_level = 7
        _, loaded = self._save_and_load(tmp_path, mock_stdscr, mutate)
        assert loaded.dungeon_level == 7

    def test_turn_count_preserved(self, tmp_path, mock_stdscr):
        def mutate(g): g.turn_count = 314
        _, loaded = self._save_and_load(tmp_path, mock_stdscr, mutate)
        assert loaded.turn_count == 314

    def test_map_content_preserved(self, tmp_path, mock_stdscr):
        _, loaded = self._save_and_load(tmp_path, mock_stdscr)
        # Room tiles should be floor, borders should be walls
        assert loaded.map[4][5] == '.'
        assert loaded.map[0][0] == '#'

    def test_rooms_preserved(self, tmp_path, mock_stdscr):
        _, loaded = self._save_and_load(tmp_path, mock_stdscr)
        assert loaded.rooms == [(2, 2, 10, 5)]

    def test_inventory_count_preserved(self, tmp_path, mock_stdscr):
    ...
    def test_equipped_weapon_preserved(self, tmp_path, mock_stdscr):
        def mutate(g):
    ...
    def test_enemy_preserved(self, tmp_path, mock_stdscr):
        def mutate(g):
    ...
    def test_messages_last_10_preserved(self, tmp_path, mock_stdscr):
        def mutate(g):
    ...
    def test_load_nonexistent_slot_raises(self, tmp_path, mock_stdscr):
        sm = SaveManager(save_dir=str(tmp_path))
        game = Game.create_minimal(24, 80, mock_stdscr)
        with pytest.raises(FileNotFoundError):
            sm.load_game(game, slot=5)

    def test_load_invalid_slot_raises(self, tmp_path, mock_stdscr):
        sm = SaveManager(save_dir=str(tmp_path))
        game = Game.create_minimal(24, 80, mock_stdscr)
        with pytest.raises(ValueError):
...
class TestDeleteSave:
    def test_delete_removes_file(self, tmp_path, mock_stdscr):
        game = make_save_game(mock_stdscr)
        sm = SaveManager(save_dir=str(tmp_path))
    ...
    def test_delete_returns_true_when_exists(self, tmp_path, mock_stdscr):
        game = make_save_game(mock_stdscr)
        sm = SaveManager(save_dir=str(tmp_path))
        sm.save_game(game, slot=3)
        assert sm.delete_save(slot=3) is True

    def test_delete_returns_false_when_absent(self, tmp_path):
        sm = SaveManager(save_dir=str(tmp_path))
        assert sm.delete_save(slot=4) is False

    def test_delete_invalid_slot_raises(self, tmp_path):
        sm = SaveManager(save_dir=str(tmp_path))
        with pytest.raises(ValueError):
...
class TestGetSaveInfo:
    def test_empty_slot_reports_not_exists(self, tmp_path):
        sm = SaveManager(save_dir=str(tmp_path))
        info = sm.get_save_info(slot=1)
        assert info['exists'] is False

    def test_filled_slot_reports_exists(self, tmp_path, mock_stdscr):
        game = make_save_game(mock_stdscr)
    ...
    def test_get_all_save_info_returns_10_entries(self, tmp_path):
        sm = SaveManager(save_dir=str(tmp_path))
        infos = sm.get_all_save_info()
        assert len(infos) == 10

    def test_get_all_save_info_slot_numbers(self, tmp_path):
        sm = SaveManager(save_dir=str(tmp_path))
        infos = sm.get_all_save_info()
