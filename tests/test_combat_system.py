...
def make_entity(name='Fighter', health=100, damage=0, defense=0, x=0, y=0):
...
def make_weapon(name='Sword', stat_boost=5):
...
class TestHitProbability:
    def test_equal_attack_defense_gives_50_percent(self):
        # diff = 0 → hit_chance = 0.5
        hit_chance = 1 / (1 + math.exp(0))
        assert abs(hit_chance - 0.5) < 1e-9

    def test_large_positive_diff_near_certainty(self):
        # diff = 50 → nearly 1.0
        hit_chance = 1 / (1 + math.exp(-50 / 5))
        assert hit_chance > 0.99

    def test_large_negative_diff_near_zero(self):
        # diff = -50 → nearly 0.0
        hit_chance = 1 / (1 + math.exp(50 / 5))
...
class TestCombatHit:
    def test_successful_hit_appends_message(self):
        cs = CombatSystem()
    ...
    def test_hit_reduces_defender_health(self):
        cs = CombatSystem()
        attacker = make_entity('Attacker', damage=5)
...
class TestCombatMiss:
    def test_miss_appends_message(self):
        cs = CombatSystem()
    ...
    def test_miss_does_not_reduce_health(self):
        cs = CombatSystem()
        attacker = make_entity('Attacker')
        defender = make_entity('Defender', health=50)
        messages = []
...
class TestCombatDefeat:
    def test_returns_true_when_defender_dies(self):
        cs = CombatSystem()
    ...
    def test_returns_false_when_defender_survives(self):
        cs = CombatSystem()
        attacker = make_entity('Attacker')
    ...
    def test_defeat_appends_defeated_message(self):
        cs = CombatSystem()
        attacker = make_entity('Killer', damage=999)
        attacker.strength = 200
...
class TestBareFists:
    def test_no_weapon_uses_random_1_3(self):
        cs = CombatSystem()
...
class TestWeaponStats:
    def test_weapon_accuracy_bonus_used_in_attack_score(self):
...
class TestPlayerAttackEnemy:
    ...
    def test_defeat_message_appended(self):
        cs, player, enemy = self._setup()
        messages = []
        with patch('random.random', return_value=1.0):
            cs.player_attack_enemy(player, enemy, messages)
    ...
    def test_loot_drops_at_enemy_position(self):
        cs, player, enemy = self._setup()
        enemy.x, enemy.y = 5, 3
        enemy.loot = [__import__('classes.item', fromlist=['Item']).Item(
            'Coin', lambda e: None)]
