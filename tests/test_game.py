...
class _Pos:
    """Minimal x/y container used as a path target in tests."""
...
class TestDistance:
    def test_chebyshev_diagonal(self, minimal_game: Game) -> None:
        a = Entity(0, 0, '@', 'A', 10, 0, 0)
        b = Entity(3, 4, 'E', 'B', 10, 0, 0)
        # Chebyshev = max(3, 4) = 4
        assert minimal_game.distance(a, b) == 4

    def test_chebyshev_horizontal(self, minimal_game: Game) -> None:
        a = Entity(2, 5, '@', 'A', 10, 0, 0)
        b = Entity(7, 5, 'E', 'B', 10, 0, 0)
        assert minimal_game.distance(a, b) == 5

    def test_distance_zero_same_position(self, minimal_game: Game) -> None:
...
class TestLine:
    def test_horizontal_line_length(self, minimal_game: Game) -> None:
        points = list(minimal_game.line(0, 0, 5, 0))
        assert len(points) == 6
        assert points[0] == (0, 0)
        assert points[-1] == (5, 0)

    def test_vertical_line(self, minimal_game: Game) -> None:
        points = list(minimal_game.line(3, 1, 3, 4))
        assert (3, 1) in points
        assert (3, 4) in points
        assert all(x == 3 for x, _y in points)

    def test_diagonal_45_degree(self, minimal_game: Game) -> None:
        points = list(minimal_game.line(0, 0, 3, 3))
        assert (0, 0) in points
        assert (3, 3) in points

    def test_single_point_line(self, minimal_game: Game) -> None:
...
class TestInRoom:
    def test_center_of_room_is_in_room(self, minimal_game: Game) -> None:
        # Room is (2, 2, 10, 5), center approx (7, 4)
        assert minimal_game.in_room(7, 4) is True

    def test_wall_tile_not_in_room(self, minimal_game: Game) -> None:
        assert minimal_game.in_room(0, 0) is False

    def test_corridor_tile_not_in_room(self, minimal_game: Game) -> None:
...
class TestGetRoom:
    def test_returns_room_for_floor_tile(self, minimal_game: Game) -> None:
        room = minimal_game.get_room(5, 4)
        assert room == (2, 2, 10, 5)

    def test_returns_none_for_wall(self, minimal_game: Game) -> None:
        assert minimal_game.get_room(0, 0) is None

    def test_returns_none_outside_all_rooms(self, minimal_game: Game) -> None:
...
class TestIsValidMove:
    def test_floor_tile_valid(self, minimal_game: Game) -> None:
        assert minimal_game.is_valid_move(6, 4) is True

    def test_wall_tile_invalid(self, minimal_game: Game) -> None:
        assert minimal_game.is_valid_move(0, 0) is False

    def test_negative_coords_invalid(self, minimal_game: Game) -> None:
        assert minimal_game.is_valid_move(-1, 0) is False
        assert minimal_game.is_valid_move(0, -1) is False

    def test_out_of_bounds_coords_invalid(self, minimal_game: Game) -> None:
        assert minimal_game.is_valid_move(100, 100) is False

    def test_stairs_tile_valid(self, minimal_game: Game) -> None:
...
class TestFindPath:
    def test_path_starts_at_origin_ends_at_goal(self, minimal_game: Game) -> None:
        target = _Pos(9, 4)
    ...
    def test_path_is_walkable(self, minimal_game: Game) -> None:
        target = _Pos(8, 4)
        path = minimal_game.find_path(minimal_game.player, target)
        assert path is not None
        for x, y in path:
    ...
    def test_no_path_through_solid_walls_returns_none(self, minimal_game: Game) -> None:
        # Target is in an unreachable walled-off area
        target = _Pos(0, 0)
        path = minimal_game.find_path(minimal_game.player, target)
        assert path is None

    def test_path_to_same_tile_is_single_element(self, minimal_game: Game) -> None:
        target = _Pos(minimal_game.player.x, minimal_game.player.y)
        path = minimal_game.find_path(minimal_game.player, target)
        assert path is not None
        assert len(path) == 1
...
class TestFlowField:
    def test_flow_field_covers_reachable_tiles(self, minimal_game: Game) -> None:
    ...
    def test_flow_field_distance_zero_at_player(self, minimal_game: Game) -> None:
        dist, _ = minimal_game.get_flow_field()
        assert dist[(minimal_game.player.x, minimal_game.player.y)] == 0

    def test_flow_field_distances_are_non_negative(self, minimal_game: Game) -> None:
        dist, _ = minimal_game.get_flow_field()
        for d in dist.values():
            assert d >= 0

    def test_flow_field_predecessor_leads_back_to_player(self, minimal_game: Game) -> None:
        ...
        while current != player_pos:
            current = prev[current]  # type: ignore[assignment]
            steps += 1
            assert steps < 200, "predecessor chain did not terminate"
        assert current == player_pos

    def test_invalidate_clears_flow_field(self, minimal_game: Game) -> None:
        minimal_game.get_flow_field()
        assert minimal_game._flow_field is not None  # type: ignore[reportPrivateUsage]
        minimal_game.invalidate_flow_field()
        assert minimal_game._flow_field is None  # type: ignore[reportPrivateUsage]

    def test_flow_field_recomputed_after_player_moves(self, minimal_game: Game) -> None:
    ...
    def test_flow_field_invalidated_when_door_opened(self, minimal_game: Game) -> None:
    ...
    def test_flow_field_invalidated_when_door_closed(self, minimal_game: Game) -> None:
        minimal_game.map[4][8] = '/'
        minimal_game.get_flow_field()
        assert minimal_game._flow_field is not None  # type: ignore[reportPrivateUsage]
    ...
    def test_get_flow_next_step_moves_toward_player(self, minimal_game: Game) -> None:
    ...
    def test_get_flow_next_step_none_when_no_path(self, minimal_game: Game) -> None:
    ...
    def test_enemy_opens_door_invalidates_flow_field(self, minimal_game: Game) -> None:
...
class TestPickupItem:
    def test_pickup_adds_to_inventory(self, minimal_game: Game) -> None:
    ...
    def test_pickup_removes_from_ground(self, minimal_game: Game) -> None:
        fn: Callable[[Entity], Any] = lambda e: None
        item = Item('Gem', fn)
        item.x, item.y = minimal_game.player.x, minimal_game.player.y
        minimal_game.items.append(item)
    ...
    def test_nothing_to_pickup_appends_message(self, minimal_game: Game) -> None:
        minimal_game.items.clear()
        minimal_game.pickup_item()
        assert any("nothing" in m.lower() for m in minimal_game.messages)

    def test_item_not_at_player_not_picked_up(self, minimal_game: Game) -> None:
        fn: Callable[[Entity], Any] = lambda e: None
        item = Item('RemoteItem', fn)
...
class TestDropBackpackItem:
    def test_drops_item_to_ground(self, minimal_game: Game) -> None:
    ...
    def test_removes_item_from_inventory(self, minimal_game: Game) -> None:
    ...
    def test_dropped_item_at_player_position(self, minimal_game: Game) -> None:
        potion = next(i for i in all_consumables if i.name == 'Health Potion')
        minimal_game.player.add_item(potion)
        minimal_game.backpack_page = 0
        minimal_game.drop_backpack_item('a')
    ...
    def test_invalid_key_appends_message(self, minimal_game: Game) -> None:
        minimal_game.player.inventory.clear()
...
class TestUseBackpackItem:
    def test_consumable_effect_applied(self, minimal_game: Game) -> None:
        minimal_game.player.inventory.clear()
        potion = next(i for i in all_consumables if i.name == 'Health Potion')
    ...
    def test_consumable_removed_after_use(self, minimal_game: Game) -> None:
    ...
    def test_equipment_item_gets_equipped(self, minimal_game: Game) -> None:
...
class TestPlayerMoveOrAttack:
    def test_moves_to_empty_floor_tile(self, minimal_game: Game) -> None:
    ...
    def test_does_not_move_into_wall(self, minimal_game: Game) -> None:
    ...
    def test_attacks_adjacent_enemy(self, minimal_game: Game) -> None:
...
class TestNextLevel:
    def test_dungeon_level_increments(self, minimal_game: Game) -> None:
        initial = minimal_game.dungeon_level
        minimal_game.next_level()
        assert minimal_game.dungeon_level == initial + 1

    def test_next_level_appends_message(self, minimal_game: Game) -> None:
        minimal_game.next_level()
        assert any('descend' in m.lower() for m in minimal_game.messages)

    def test_next_level_generates_new_map(self, minimal_game: Game) -> None:
        minimal_game.next_level()
...
class TestPreviousLevel:
    def test_dungeon_level_decrements(self, minimal_game: Game) -> None:
        minimal_game.dungeon_level = 3
        minimal_game.previous_level()
        assert minimal_game.dungeon_level == 2

    def test_previous_level_appends_message(self, minimal_game: Game) -> None:
...
class TestHandlePlayerDeath:
    def test_sets_game_over_flag(self, minimal_game: Game) -> None:
        minimal_game.handle_player_death()
        assert minimal_game.game_over is True

    def test_sets_quit_flag(self, minimal_game: Game) -> None:
        minimal_game.handle_player_death()
        assert minimal_game.quit is True

    def test_health_clamped_to_zero(self, minimal_game: Game) -> None:
...
class TestCheckCollisions:
    def test_auto_picks_up_item_at_player_position(self, minimal_game: Game) -> None:
        fn: Callable[[Entity], Any] = lambda e: None
        item = Item('Floor Item', fn)
        item.x, item.y = minimal_game.player.x, minimal_game.player.y
        minimal_game.items.append(item)
    ...
    def test_leaves_items_at_other_positions(self, minimal_game: Game) -> None:
        fn: Callable[[Entity], Any] = lambda e: None
        item = Item('Distant Item', fn)
        item.x, item.y = 0, 0
        minimal_game.items.append(item)
        minimal_game.check_collisions()
        assert item in minimal_game.items
...
class TestCloseDoor:
    def _place_open_door(self, game: Game, dx: int, dy: int) -> tuple[int, int]:
        tx, ty = game.player.x + dx, game.player.y + dy
        game.map[ty][tx] = '/'
        return tx, ty

    def test_closes_open_door(self, minimal_game: Game) -> None:
    ...
    def test_already_closed_door(self, minimal_game: Game) -> None:
    ...
    def test_no_door_there(self, minimal_game: Game) -> None:
        # Target is a plain floor tile
        minimal_game.close_door(1, 0)
        assert any("no door" in m.lower() for m in minimal_game.messages)

    def test_out_of_bounds(self, minimal_game: Game) -> None:
    ...
    def test_blocked_by_enemy(self, minimal_game: Game) -> None:
        tx, ty = self._place_open_door(minimal_game, 1, 0)
    ...
    def test_close_door_processes_turn(self, minimal_game: Game) -> None:
...
class TestRestUntilHealed:
    def test_already_full_does_not_rest(self, minimal_game: Game) -> None:
    ...
    def test_enemy_in_view_prevents_rest(self, minimal_game: Game) -> None:
        p = minimal_game.player
        p.health = 1.0
    ...
    def test_rest_heals_hp(self, minimal_game: Game) -> None:
        p = minimal_game.player
    ...
    def test_rest_recovers_mana(self, minimal_game: Game) -> None:
        p = minimal_game.player
        p.health = p.max_health
        p.mana = p.max_mana - 1
        p.psi = p.max_psi
    ...
    def test_rest_recovers_psi(self, minimal_game: Game) -> None:
        p = minimal_game.player
        p.health = p.max_health
        p.mana = p.max_mana
        p.psi = p.max_psi - 1
    ...
    def test_rest_interrupted_by_attack(self, minimal_game: Game) -> None:
        p = minimal_game.player
        p.health = 50.0
        p.mana = p.max_mana
        p.psi = p.max_psi
        ...
        def patched_process() -> None:
            call_count[0] += 1
            original_process()
            minimal_game.player.health -= 5  # simulate hit

        minimal_game.process_turn = patched_process  # type: ignore[method-assign]
        minimal_game.enemies.clear()
        minimal_game.allow_enemy_spawning = False
        minimal_game.stdscr = None
        minimal_game.rest_until_healed()
    ...
    def test_rest_mode_flag_cleared_after_rest(self, minimal_game: Game) -> None:
        p = minimal_game.player
        p.health = p.max_health - 1
        p.mana = p.max_mana
        p.psi = p.max_psi
