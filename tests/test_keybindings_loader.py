...
from classes.keybindings_loader import _DEFAULTS  # type: ignore[reportPrivateUsage]
from classes.keybindings_loader import _NAMED_KEYS  # type: ignore[reportPrivateUsage]
from classes.keybindings_loader import _parse_key  # type: ignore[reportPrivateUsage]
...
class TestParseKey:
    def test_single_char_returns_ordinal(self) -> None:
    ...
    def test_named_key_up(self) -> None:
        assert _parse_key("KEY_UP") == curses.KEY_UP

    def test_named_ctrl_w(self) -> None:
        assert _parse_key("CTRL_W") == 23

    def test_all_named_keys_resolve(self) -> None:
    ...
    def test_non_string_raises(self) -> None:
        with pytest.raises(ValueError, match="must be a string"):
            _parse_key(65)  # type: ignore[arg-type]

    def test_multi_char_non_named_raises(self) -> None:
        with pytest.raises(ValueError, match="Unknown key name"):
            _parse_key("kk")
...
class TestLoadKeybindingsFromFile:
    def test_returns_keybindings_instance(self) -> None:
        kb = load_keybindings()
        assert isinstance(kb, Keybindings)

    def test_all_actions_populated(self) -> None:
        kb = load_keybindings()
        for action in _DEFAULTS:  # type: ignore[reportUnknownVariableType]
            keys: frozenset[int] = getattr(kb, action)
            assert isinstance(keys, frozenset), f"{action} is not a frozenset"
            assert len(keys) > 0, f"{action} has no keys bound"

    def test_default_movement_contains_expected_keys(self) -> None:
        kb = load_keybindings()
    ...
    def test_default_diagonal_keys(self) -> None:
        kb = load_keybindings()
    ...
    def test_default_action_keys(self) -> None:
        kb = load_keybindings()
    ...
    def test_wait_contains_numpad_5_and_b2(self) -> None:
        kb = load_keybindings()
...
class TestLoadKeybindingsMissingFile:
    ...
    def test_missing_file_all_actions_populated(self, tmp_path: Path) -> None:
        kb = load_keybindings(path=str(tmp_path / "nonexistent.json"))
...
class TestLoadKeybindingsCustomFile:
    ...
    def test_override_single_action(self, tmp_path: Path) -> None:
    ...
    def test_unmentioned_actions_keep_defaults(self, tmp_path: Path) -> None:
        path = self._write(tmp_path, {"quit": ["X"]})
        kb = load_keybindings(path)
        assert ord("X") in kb.quit
        assert ord("k") in kb.move_n   # untouched

    def test_override_movement_keys(self, tmp_path: Path) -> None:
    ...
    def test_named_key_in_override(self, tmp_path: Path) -> None:
        path = self._write(tmp_path, {"wait": ["KEY_B2"]})
        kb = load_keybindings(path)
        assert curses.KEY_B2 in kb.wait

    def test_multiple_keys_per_action(self, tmp_path: Path) -> None:
        path = self._write(tmp_path, {"pickup": ["g", "p"]})
        kb = load_keybindings(path)
        assert ord("g") in kb.pickup
        assert ord("p") in kb.pickup

    def test_invalid_key_name_raises(self, tmp_path: Path) -> None:
        path = self._write(tmp_path, {"inventory": ["INVALID"]})
        with pytest.raises(ValueError, match="Unknown key name"):
            load_keybindings(path)

    def test_non_list_binding_raises(self, tmp_path: Path) -> None:
        path = self._write(tmp_path, {"inventory": "i"})
        with pytest.raises(ValueError, match="must be a list"):
            load_keybindings(path)

    def test_empty_file_object_uses_all_defaults(self, tmp_path: Path) -> None:
...
class TestDuplicateDetection:
    ...
    def test_duplicate_across_actions_raises(self, tmp_path: Path) -> None:
    ...
    def test_duplicate_message_names_both_actions(self, tmp_path: Path) -> None:
    ...
    def test_duplicate_named_key_raises(self, tmp_path: Path) -> None:
        path = self._write(tmp_path, {"move_n": ["KEY_UP"], "move_s": ["KEY_UP"]})
        with pytest.raises(ValueError, match="Duplicate key bindings"):
            load_keybindings(path)

    def test_no_duplicate_in_defaults(self) -> None:
    ...
    def test_non_printable_code_shown_as_code_number(self, tmp_path: Path) -> None:
    ...
    def test_printable_char_shown_as_repr(self, tmp_path: Path) -> None:
